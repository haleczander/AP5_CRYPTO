...
Malgré le scepticisme de la communauté scientifique, DUAL_EC_DRBG a été adopté en 2006 par le NIST (National Institute of Standards and Technology) dans sa norme SP 800-90A, aux côtés de trois autres algorithmes de génération aléatoire. Il a également été standardisé par l'ANSI et l'ISO, gagnant ainsi une portée internationale. Pendant sept ans, DUAL_EC_DRBG est resté l'un des générateurs approuvés, jusqu'à ce que de graves doutes sur sa sécurité ne conduisent finalement à son retrait officiel en 2014.

Il faut noter qu'avant même sa standardisation, des experts avaient relevé des faiblesses et émis des critiques. En 2006, par exemple, le cryptologue Kristian Gjøsteen montrait que certains aspects de DUAL_EC_DRBG n'étaient "pas cryptographiquement solides", identifiant un biais dans la production de bits aléatoires. De plus, le générateur était extrêmement lent, environ cent fois plus lent que d'autres PRNG basés sur des fonctions de hachage, ce qui le rendait peu pratique dans de nombreux contextes. Malgré ces signaux d'alerte (absence de preuve de sécurité formelle, biais statistique, performances médiocres), le standard a été finalisé sans correction notable de ces points.

Dans la suite de ce rapport, nous allons expliquer le fonctionnement interne de DUAL_EC_DRBG, ses fondements mathématiques et l’hypothèse de sécurité qui était censée le protéger. Nous reviendrons ensuite sur la découverte de sa vulnérabilité majeure, suspectée d’être une porte dérobée intentionnelle, et sur le rôle controversé de la NSA dans cette affaire. Le cas RSA BSAFE et le contrat liant RSA à la NSA sera également abordé. Enfin, certains enseignements à tirer de l’affaire DUAL_EC_DRBG en matière de sécurité et de standardisation cryptographique seront avancés, avant de conclure sur les leçons de cette histoire.
...
Le fonctionnement peut être décrit simplement en deux phases à chaque itération: une phase de mise à jour de l’état interne et une phase de production de la sortie psuedo-aléatoire. La figure 1 ci-dessous illustre ce processus.
![Figure 1 : Schéma simplifié du fonctionnement de DUAL_EC_DRBG](1_DUAL_EC_DRBG_fonctionnement.png)

Chaque tour prend l’état interne courant s_i et le met à jour via une multiplication par P. Le nouvel état $s_{i+1}$ sert ensuite à calculer la sortie pseudo-aléatoire en le multipliant au point Q et en extrayant l’abscisse du point résultant. Une troncature est appliquée pour ne conserver que 30 octets de sortie.  

A partir d’un état interne initial (issu d’une graine aléatoire de départ), le générateur effectue les opérations suivantes à chaque demande de nombres aléatoires:

1. Mise à jour de l’état interne : calcul d'un nouveau point en multipliant P par l’état courant. On extrait ensuite l’abscisse x de ce point $P^{s_i}$ qui devient la nouvelle valeur de l’état interne $s_{i+1}$.

2. Génération de la sortie aléatoire : à partir du nouvel état $s_{i+1}$, le générateur multiplie cette fois Q par $s_{i+1}$. On obtient le point $Q^{s_{i+1}}$. L'abscisse de ce point sera tronquée afin d’obtenir des bits aléatoires utilisables. Dans la spécification de NIST P-256, ne sont conservés que les 240 bits de poids faible (soit 30 octets) comme sortie aléatoire. Si davantage de bits sont requis, le générateur calculera autant de nouveaux états et de nouvelles sorties pour fournir le volume de données demandé.

Le point P fait évoluer l'état interne, tandis que le point Q produit la sortie. La troncature de 16 bits, quant à elle, est censée éviter qu’un attaquant puisse remonter trop facilement de la sortie vers l’état interne exact, nous verrons plus loin que c’est précisément là que le bât blesse.

Il convient de noter que, bien que DUAL_EC_DRBG respecte ce schéma général, il propose aussi des fonctionnalités optionnelles comme l’additional input (apport de données supplémentaires pour re-mélanger l’état à chaque tour) ou des opérations de ré-initialisation périodique de l'état interne avec une nouvelle valeur aléatoire. Dans la suite, nous nous concentrerons sur le fonctionnement de base sans additional input, car c’est dans ce mode que la faille de sécurité est la plus flagrante.
![Figure 2: schéma de DUAL_EC_DRBG intégrant l'additonal input](DUAL_EC_DRBG_plus.png)
...
Il est important de souligner que l’attrait de DUAL_EC_DRBG résidait dans cette promesse de sécurité "réductionniste" basée sur un problème complexe. Contrairement à d’autres PRNG dont la robustesse se fonde sur des considérations plus empiriques ou sur la difficulté à inverser des fonctions de hachage (ce qui n’est pas une garantie mathématiquement prouvée), on espérait pouvoir apporter une preuve de sécurité pour DUAL_EC_DRBG. 
...
### Travail antérieurs à la diffusion de DUAL_EC_DRBG
En mai 1997, Young et Yung présentent à Eurocrypt leur article " Kleptography: Using Cryptography Against Cryptography ", où ils montrent comment intégrer une porte dérobée asymétrique dans le protocole Diffie–Hellman. Le mécanisme utilisé dans l’EC-DRBG en sera plus tard une simple variante.
En août 1997, à Crypto, ils généralisent cette approche dans " The Prevalence of Kleptographic Attacks on Discrete-Log Based Cryptosystems ", introduisant le discrete log kleptogram, qui servira de base directe au backdoor de l’EC-DRBG.
